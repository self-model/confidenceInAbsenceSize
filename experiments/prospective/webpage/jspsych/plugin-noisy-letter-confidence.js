  ...
  class NoisyLetterPlugin {
    ...
    trial(display_element, trial) {
      ...
      var rate_confidence = (confidence) => {
        ...
        p.stroke(0);
        p.strokeWeight(4);
        p.line(window.innerWidth/3, window.innerHeight/4, window.innerWidth/3, window.innerHeight*3/4)
        p.pop()
        ...
        p.text(`100% certain the letter was ${window.decision}`,window.innerWidth/3+40,window.innerHeight/4)
        p.text(`Guessing the letter was ${window.decision}` ,window.innerWidth/3+40,window.innerHeight*3/4)
        p.pop()
        ...
        if (window.mouseMoved) {
          ...
          p.strokeWeight(0.5);
          p.fill(255)
          p.ellipse(window.innerWidth/3,window.dial_position,20)
          p.pop()
        }
        ...
        p.keyReleased = () => {
          ...
          if ((key=='g' | key=='f') & trial.RT==Infinity) {
            // only regard relevant key presses during the response phase
              ...
              window.decision=trial.response==trial.choices[1]? 'present' : 'absent';
              // data saving
              window.trial_data = {
